...
class PubspecUtils {
  ...
  static final _mapSep = _PubValue<String>(() {
    ...
    if (yaml.containsKey('rgb_cli')) {
      if ((yaml['rgb_cli'] as Map).containsKey('separator')) {
        return (yaml['rgb_cli']['separator'] as String?) ?? '';
      }
    }
  ...
  static final _extraFolder = _PubValue<bool?>(
    () {
      try {
        var yaml = pubspecJson;
        if (yaml.containsKey('rgb_cli')) {
          if ((yaml['rgb_cli'] as Map).containsKey('sub_folder')) {
            return (yaml['rgb_cli']['sub_folder'] as bool?);
          }
        }
