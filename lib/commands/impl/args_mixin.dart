...
mixin ArgsMixin {
  final List<String> _args = RgbCli.arguments;
...
List<String> _getArgs() {
  var args = List.of(RgbCli.arguments);
  _removeDefaultArgs(args);
...
int _getIndexArg(String arg) {
  return RgbCli.arguments.indexWhere((element) => element == arg);
}

String _getArg(String arg) {
  var index = _getIndexArg(arg);
  if (index != -1) {
    if (index + 1 < RgbCli.arguments.length) {
      index++;
      return RgbCli.arguments[index];
    } else {
